...
_ZHEN_DEV_DATASETS = [
            LOCATION_OF_DATA+'val.art.padded',
                LOCATION_OF_DATA+'val.abs.padded',
                    LOCATION_OF_DATA+'val.art.padded'
                    ]
...
class DelibZhenWmt17(TranslateProblem):
    ...
    def generate_samples(self, data_dir, tmp_dir, train):
        ...
        return tri_vocabs_token2id_generator(datapath[0], datapath[1], datapath[2], source_vocab, target_vocab, text_encoder.EOS_ID)

    def source_data_files(self, dataset_split):
      train = dataset_split == problem.DatasetSplit.TRAIN
      return _ZHEN_TRAIN_DATASETS if train else _ZHEN_DEV_DATASETS    
    ...
    def hparams(self, defaults, unused_model_hparams):
        ...
        p.modality = {"inputs": modalities.ModalityType.SYMBOL,
                   "firstP": modalities.ModalityType.SYMBOL,
                   "targets": modalities.ModalityType.SYMBOL}
        p.vocab_size = {"inputs": source_vocab_size,
                     "firstP": target_vocab_size,
                     "targets": target_vocab_size}
        '''
        ...
        if self.has_inputs:
            source_vocab_size = self._encoders["inputs"].vocab_size
            p.modality = {
              "inputs": (modalities.ModalityType.SYMBOL, source_vocab_size),
              "firstP": (modalities.ModalityType.SYMBOL, target_vocab_size)
            }
        
        p.target_modality = (modalities.ModalityType.SYMBOL, target_vocab_size)
        if self.has_inputs:
...
def zhen_wmt17_transformer_big_v1():
    ...
    #hparams.add_hparam("init_checkpoint", "/exps/jive/trans-ape-bpe40-allover3-totrans/model.ckpt-248797")
    #hparams.add_hparam("init_checkpoint", "/exps/jive/ape-base/model.ckpt-383901")
    return hparams
...
def transformer_delib_big_v2():
    ...
    hparams.add_hparam("update_delib_only", False)
    #hparams.add_hparam("init_checkpoint", "/exps/jive/trans-mm-en-fr-big-v1/model.ckpt-20000")
    hparams.add_hparam("init_checkpoint", "/exps/jive/ape-base/model.ckpt-383901")
