...
def transformer_decode(decoder_function,
  ...
  if (common_layers.is_xla_compiled() and
    ...
    decoder_output[0] = tf.expand_dims(decoder_output[0], axis=2)
    return decoder_output
...
class Transformer(t2t_model.T2TModel):
  ...
  def decode(self,
             ...
             decode_loop_step=None,
             nonpadding=None,
             losses=None, indices=None, firstP=None, firstP_decoder_self_attention_bias=None,
             **kwargs):
    ...
    return transformer_decode(
        ...
        decode_loop_step=decode_loop_step, nonpadding=nonpadding, losses=losses, indices=indices, firstP=firstP, firstP_decoder_self_attention_bias=firstP_decoder_self_attention_bias,**kwargs)
  ...
  def body(self, features):
    ...
    firstP_raw = features.get("firstP_raw")

    firstP_raw.set_shape([None, None])
    firstP_raw = tf.expand_dims(firstP_raw, -1)
    ...
    firstP_output, firstP_attention_bias = self.encode(
          firstP, target_space, hparams, features=features, losses=losses)
    ...
    decoder_output = self.decode(
        ...
        losses=losses, indices=firstP_raw, firstP=firstP_output, firstP_decoder_self_attention_bias=firstP_attention_bias,
        **decode_kwargs
        )
    
    logits = decoder_output[2]
    
    #JI: formulas 6 and 7; ugly hardcoded batch size 30
    nonpad = features_to_nonpadding(features, "targets")
    nonpad = common_attention.embedding_to_padding(targets)
    nonpad_mask = 1.0 - nonpad
    nonpad_mask = tf.expand_dims(nonpad_mask, -1)

    
    logits_mask = 1.0 - (decoder_self_attention_bias / -1e9)
    logits_mask = tf.transpose(logits_mask, perm=[1, 2, 3, 0])
    
    trg_len = common_layers.shape_list(logits)[1]
    logits_ext = tf.reshape(logits, [30, -1])
    logits_ext = tf.tile(logits_ext,[1, trg_len])
    logits_ext = tf.reshape(logits_ext, [30, trg_len, trg_len, -1])
    logits_ext = tf.multiply(logits_ext, logits_mask)
    coverage = tf.reduce_sum(logits_ext, 2)
    coverage = tf.subtract(coverage, logits)

    cov = tf.minimum(logits, coverage)
    cov = tf.reduce_sum(cov, -1)
    cov = tf.expand_dims(cov, -1)
    cov_masked  = tf.multiply(cov, nonpad_mask)
    
   losses.append([tf.reduce_sum(cov_masked)])
    ...
    decoder_output[0] = tf.reshape(decoder_output[0], targets_shape)
    ret = decoder_output
    if losses:
  ...
  def _fast_decode_tpu(self,
    ...
    if self.has_input:
      ...
      firstP_modality = self._problem_hparams.modality["firstP"]
      firstP_vocab_size = self._problem_hparams.vocab_size["firstP"]
      fp_modality_name = hparams.name.get(
          "firstP",
          modalities.get_name(firstP_modality))(hparams, firstP_vocab_size)
      with tf.variable_scope(fp_modality_name):
        bottom = hparams.bottom.get("firstP",
                                    modalities.get_bottom(firstP_modality))
        firstP = dp(bottom, firstP, hparams, firstP_vocab_size)
      with tf.variable_scope("body", reuse=True):
        firstP_output, firstP_decoder_attention_bias = dp(
            self.encode,
            inputs,
            features["target_space_id"],
            hparams,
      ...
      firstP_output = firstP_output[0]
      firstP_decoder_attention_bias = firstP_decoder_attention_bias[0]
  ...
  def _fast_decode(self,
    ...
    if self.has_input:
      ...
      firstP = features["firstP"]
      firstP.set_shape([None, None])
      firstP = tf.expand_dims(firstP, axis=2)
      firstP = tf.expand_dims(firstP, axis=2)
      firstP = tf.expand_dims(firstP, axis=1)
      if len(firstP.shape) < 5:
        firstP = tf.expand_dims(firstP, axis=4)
      ...
      fp_modality_name = hparams.name.get(
          "firstP",
          modalities.get_name(firstP_modality))(hparams, firstP_vocab_size)
      with tf.variable_scope(fp_modality_name, reuse=tf.AUTO_REUSE):
        bottom = hparams.bottom.get("firstP",
                                    modalities.get_bottom(firstP_modality))
        firstP = dp(bottom, firstP, hparams, firstP_vocab_size)
      #with tf.variable_scope("body", reuse=True):
    ...
    def symbols_to_logits_fn(ids, i, cache):
      ...
      with tf.variable_scope("body"):
        body_outputs = dp(
            ...
            firstP=cache.get("firstP_output"), 
            firstP_decoder_self_attention_bias=cache.get("firstP_decoder_attention_bias"))
      ...
      ret = tf.squeeze(logits, axis=[1, 2, 3])
      if partial_targets is not None:
        # the position is within the given partial targets, we alter the
        # logits to always return those values.
        # A faster approach would be to process the partial targets in one
        # iteration in order to fill the corresponding parts of the cache.
        # This would require broader changes, though.
    ...
    ret = fast_decode(
        encoder_output=encoder_output,
        encoder_decoder_attention_bias=encoder_decoder_attention_bias,
        indices=firstP_raw, firstP=firstP_output, firstP_decoder_self_attention_bias=firstP_decoder_attention_bias,        
...
def fast_decode_tpu(encoder_output,
  ...
  if encoder_output is not None:
    ...
    for layer in range(num_layers):
      layer_name = "layer_%d" % layer
      with tf.variable_scope("%sdecoder/%s/dec_attention/multihead_attention"
                             % (scope_prefix, layer_name)):
        k_encdec = common_attention.compute_attention_component(
            encoder_output,
            key_channels,
...
def fast_decode(encoder_output,
                encoder_decoder_attention_bias,
                indices, firstP, firstP_decoder_self_attention_bias,
                symbols_to_logits_fn,
                hparams,
                decode_length,
                vocab_size,
  ...
  cache.update({
      "layer_%d2nd" % layer: {  # pylint: disable=g-complex-comprehension
          "k":
              common_attention.split_heads(
                  tf.zeros([batch_size, 0, key_channels]), hparams.num_heads),
          "v":
  ...
  if firstP is not None:
    for layer in range(num_layers):
      layer_name = "layer_%d" % layer
      with tf.variable_scope("%sdecoder/%s/firstPdec_attention/multihead_attention"
                             % (scope_prefix, layer_name)):
        k_encdec = common_attention.compute_attention_component(
            firstP,
            key_channels,
    ...
    cache["firstP_output"] = firstP
    cache["firstP_decoder_attention_bias"] = firstP_decoder_self_attention_bias
    ...
    for layer in range(num_layers):
      ...
      with tf.variable_scope("%sdecoder/%s/encdec_attention/multihead_attention"
        ...
        v_encdec = common_attention.compute_attention_component(
            ...
            vars_3d_num_heads=vars_3d_num_heads)
        v_encdec = common_attention.split_heads(v_encdec, hparams.num_heads)
      cache[layer_name+'2nd']["k_encdec"] = k_encdec
      cache[layer_name+'2nd']["v_encdec"] = v_encdec
...
def transformer_decoder(decoder_input,
                        ...
                        chunk_number=None, indices=None,
                        firstP=None, firstP_decoder_self_attention_bias=None):
  ...
  """
  firstP_att = firstP
  #print(tf.shape(firstP_decoder_self_attention_bias))
  #exit()
  x = decoder_input
  ...
  with tf.variable_scope(name):
    for layer in range(hparams.num_decoder_layers or hparams.num_hidden_layers):
      ...
      with tf.variable_scope(layer_name):
        ...
        print(firstP)
        if firstP is not None:
          with tf.variable_scope("firstPdec_attention"):
            y = common_attention.multihead_attention(
                common_layers.layer_preprocess(
                    x, hparams, layer_collection=layer_collection),
                firstP,
                firstP_decoder_self_attention_bias,
                hparams.attention_key_channels or hparams.hidden_size,
                hparams.attention_value_channels or hparams.hidden_size,
                hparams.hidden_size,
                hparams.num_heads,
            ...
            self_y = y
            #firstP_att = tf.Print(firstP_att,[tf.shape(firstP_att)], message='attimage', summarize=200000)
            #print(self_y.get_shape())
            #exit()
            x = common_layers.layer_postprocess(x, y, hparams)
        print(encoder_output)
        if encoder_output is not None:
          layer_cache = cache[layer_name+'2nd'] if cache is not None else None
    ...
    x = common_layers.layer_preprocess(
        x, hparams, layer_collection=layer_collection)
    
    with tf.variable_scope(name):
      #JI: formula 8 
      p_gen_logits = tf.layers.dense(self_y, 1, activation=None, use_bias=False, kernel_initializer=tf.initializers.orthogonal, name="w1_dec_gate")
      #self_y = tf.Print(self_y,[tf.shape(self_y)], message='logits', summarize=800)
      logits = tf.matmul(self_y, firstP_att, transpose_b=True)
      logits = tf.nn.softmax(logits, name="attention_weights1")
      
      trg_len = common_layers.shape_list(logits)[1]
      #JI: formula 4 ugly hardcoded batch size 30 and source length 451
      logits_mask = 1.0 - (decoder_self_attention_bias / -1e9)
      logits_mask = tf.transpose(logits_mask, perm=[1, 2, 3, 0])
       
      logits_ext = tf.reshape(logits, [30, -1])
      logits_ext = tf.tile(logits_ext,[1, trg_len])
      logits_ext = tf.reshape(logits_ext, [30, trg_len, trg_len, 451])
      logits_ext = tf.multiply(logits_ext, logits_mask)
      coverage = tf.reduce_sum(logits_ext, 2)
      coverage = tf.subtract(coverage, logits)
      coverage_features = tf.layers.dense(coverage, 451, use_bias=False, name="w_c", kernel_initializer=tf.initializers.orthogonal)
      #JI: formula 5
      logits2 = logits + coverage_features
      
      weights = tf.nn.softmax(logits2, name="attention_weights")

    return [x, p_gen_logits, weights, indices]
