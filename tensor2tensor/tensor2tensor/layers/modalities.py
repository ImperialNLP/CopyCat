...
def symbol_top(body_output, targets, model_hparams, vocab_size):
  ...
  print(body_output)
  #exit()
  def top_sub(body_output, targets, model_hparams, vocab_size):
    """Generate logits.
  ...
  in_top_sub = body_output[0]
  #JI getting p_gen
  logits_dec = top_sub(in_top_sub, targets, model_hparams, vocab_size)
  p_gens_logits = body_output[1]
  p_gens = tf.sigmoid(p_gens_logits, name="p_gens")

  dist_dec = tf.nn.softmax(logits_dec)
  dist_dec = tf.squeeze(dist_dec, [2,3])
  enc = body_output[2]
  enc.set_shape([None, None, None])
  
  indices = body_output[3]
  indices.set_shape([None, None, 1])
  out_list = []
  
  #JI: match probas over source to the vocab distribution; ugly hardcoded natch_size 30
  for i in range(30):
    out_prep = tf.scatter_nd(indices[i], tf.transpose(enc[i], perm=(1, 0)), tf.shape(tf.transpose(dist_dec[i], perm=(1, 0))))
    out_list.append(tf.transpose(out_prep, perm=(1, 0)))
  
  dist_enc = tf.stack(out_list)  
  p_ones = tf.ones(tf.shape(p_gens))
  
  p_pointer=tf.subtract(p_ones, p_gens)
  #p_pointer = tf.Print(p_pointer,[p_pointer], message='p_pointer', summarize=30000)
  comp1 = tf.multiply(p_gens, dist_dec)
  comp2 = tf.multiply(p_pointer, dist_enc)
  
  #JI: formula 3
  new_dec_out = tf.add(comp1, comp2)
  
  logits=new_dec_out
  
  logits = tf.expand_dims(logits, 2)
  logits = tf.expand_dims(logits, 2)
  logits.set_shape([None, None, 1, 1, vocab_size])
