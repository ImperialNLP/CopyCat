...
def smoothing_cross_entropy(logits,
  ...
  with tf.name_scope("smoothing_cross_entropy", values=[logits, labels]):
    ...
    if gaussian and confidence > 0.0:
      ...
      '''
      #JI: we do not use smoothing here
      soft_targets = tf.one_hot(  
          tf.cast(labels, tf.int32),
          depth=vocab_size)
    ...
    #    logits=logits, labels=soft_targets)
    #xentropy = tf.nn.softmax_cross_entropy(logits=logits, labels=soft_targets)
    #return xentropy - normalizing
    #softmax = tf.nn.softmax(logits)
    #xentropy = -tf.reduce_sum(soft_targets*tf.log(logits))
    #xentropy = -tf.reduce_sum(soft_targets*tf.log(tf.clip_by_value(logits,1e-10,1.0)))
    xentropy = -tf.reduce_sum(soft_targets*tf.log(logits + 1e-10))
    return xentropy
